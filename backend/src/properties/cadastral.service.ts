...
export class CadastralService {
  ...
  async lookup(city: string, street: string, houseNumber: string) {
    if (!city || !street || !houseNumber) {
      throw new HttpException('עיר, רחוב ומספר בית הם שדות חובה.', HttpStatus.BAD_REQUEST);
    }
    ...
    try {
      ...
      const geoResponse = await fetch(`https://api.govmap.gov.il/position/json?${params}`, {
        headers: {
      ...
      if (geoData.ResultType !== 1 || !geoData.X || !geoData.Y) {
        throw new HttpException('לא הצלחנו לאתר את הקואורדינטות של הכתובת הזו.', HttpStatus.NOT_FOUND);
      }

      // Step 2: Intersect X, Y with PARCEL_ALL layer
      this.logger.log(`Cadastral Lookup: Intersecting X=${geoData.X}, Y=${geoData.Y}`);
      ...
      if (!intersectResponse.ok) {
        throw new Error(`GovMap intersect failed: HTTP ${intersectResponse.status}`);
      }

      const intersectData = (await intersectResponse.json()) as GovMapIntersectResponse;
      const parcels = intersectData.data?.PARCEL_ALL;

      if (!parcels || parcels.length === 0) {
        throw new HttpException('לא נמצאו נתוני גוש/חלקה רשמיים בנקודה זו.', HttpStatus.NOT_FOUND);
      }
      ...
      throw new HttpException('שגיאה בתקשורת מול מאגרי משרד הפנים.', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
}

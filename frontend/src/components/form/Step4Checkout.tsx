...
export function Step4Checkout({ data, isSubmitting, onSubmit, onChange }: Props) {
  ...
  return (
    <div className="space-y-10 animate-fade-in-up">
      ...
      {/* Summary Card */}
      <div className="border border-white/[0.08] bg-white/[0.01] rounded-xl p-8 space-y-6">
        <div className="space-y-1">
          <p className="text-[10px] uppercase tracking-widest text-slate-500">כתובת הנכס</p>
          <p className="text-lg text-white" style={{ fontFamily: "var(--font-serif)" }}>
            {addressString || "לא הוזן"}
          </p>
        </div>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 pt-4 border-t border-white/[0.06]">
          <div className="space-y-1">
            <p className="text-[10px] uppercase tracking-widest text-slate-500">סוג נכס</p>
            <p className="text-sm text-slate-200">{propertyType || "-"}</p>
          </div>
          <div className="space-y-1">
            <p className="text-[10px] uppercase tracking-widest text-slate-500">חדרים</p>
            <p className="text-sm text-slate-200">{rooms || "-"}</p>
          </div>
          <div className="space-y-1">
            <p className="text-[10px] uppercase tracking-widest text-slate-500">מחיר מבוקש</p>
            <p className="text-sm text-slate-200">{askingPrice ? `₪${askingPrice}` : "-"}</p>
          </div>
        </div>
      </div>
      ...
      <div className="text-center py-6">
        <p className="text-[11px] uppercase tracking-widest text-slate-400 mb-2">מחיר הדוח</p>
        <p className="text-5xl font-bold text-[#00C896] mb-2" style={{ fontFamily: "var(--font-serif)" }}>₪230</p>
        <p className="text-xs text-slate-500">כולל מע״מ</p>
      </div>

  ...
  return (
    <header className="w-full bg-[#060E1C] border-b border-white/10 pb-6 mb-8 text-right">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4">
        ...
        <div className="text-right sm:text-left text-xs text-slate-400 self-end sm:self-auto">
          <span className="bg-white/5 border border-white/10 px-3 py-1.5 rounded-lg font-mono text-teal-400 inline-block">
            {reportNumber}
          </span>
      ...
      <div>
        ...
        <p className="text-slate-300 text-sm mt-1 flex flex-wrap items-center gap-2">
          <span>{address}</span>
          {projectName && (
            <span className="text-teal-400 text-xs border border-teal-500/30 px-2 py-0.5 rounded shrink-0">
              {projectName}
            </span>
          )}
        </p>

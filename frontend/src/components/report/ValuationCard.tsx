    ...
    <div className="p-8 rounded-2xl border border-white/[0.08] bg-[#060E1C]/80 backdrop-blur-md space-y-8 animate-fade-in-up">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/[0.06] pb-6">
        <div>
          ...
          <h3
            className="text-xl sm:text-2xl font-bold text-white tracking-wide flex items-center flex-wrap gap-2"
        ...
        {/* Confidence Badge */}
        <div className="flex items-center gap-2 bg-white/[0.03] border border-white/[0.08] px-3.5 py-1.5 rounded-full text-xs text-slate-300 self-start sm:self-auto shrink-0">
      ...
      {/* Fairness Banner */}
      <div
        className={`p-4 rounded-xl border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 ${style.bg}`}
      >
      ...
      <div className="space-y-4 pt-2">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-1 sm:gap-2 text-[11px] sm:text-xs text-slate-400 font-medium tracking-wider">
          <span>טווח נמוך: {formatPrice(minValue)}</span>
          <span className="text-[#00C896] font-bold">
            שווי משוער: {formatPrice(estimatedValue)}
          </span>
          <span>טווח גבוה: {formatPrice(maxValue)}</span>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-1 text-[11px] text-slate-500 pt-1">
      ...
      {comparableDeals && comparableDeals.length > 0 && (
        <div className="space-y-3 pt-4 border-t border-white/[0.06]">
          <h4 className="text-xs uppercase tracking-widest text-slate-400 font-medium flex items-center justify-between flex-wrap gap-2">
            ...
            <table className="w-full text-right text-xs min-w-[480px]">
              ...
              <tbody className="divide-y divide-white/[0.04] text-slate-300">
                {comparableDeals.map((deal, idx) => (
                  ...
                  >
                    <td className="py-3 px-3 text-slate-400 whitespace-nowrap">{deal.dealDate}</td>
                    <td className="py-3 px-3 whitespace-nowrap">{deal.rooms}</td>
                    <td className="py-3 px-3 whitespace-nowrap">{deal.sqm} מ"ר</td>
                    <td
                      className="py-3 px-3 font-semibold text-white whitespace-nowrap"
                      style={{ fontFamily: "var(--font-serif)" }}
                    >
                      {formatPrice(deal.price)}
                    </td>
                    <td className="py-3 px-3 text-[#00C896] whitespace-nowrap">
